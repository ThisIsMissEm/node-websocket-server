...
function Server(o) {
  ...
  var self = this;
  manager.on('attach', function(connection) {
    // emit URL specific connection event to allow multiple endpoints (@kschzt)
    // eg. offer a service on /myService and listen for 'connection/myService'.
    self.emit('connection' + connection._req.url, connection);
