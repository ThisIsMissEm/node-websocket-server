...
function Connection(server, req, socket, upgradeHead){
  if (server.options && server.options.debug) {
...
/*-----------------------------------------------
  Writing data:
-----------------------------------------------*/
function writeSocket(socket, data, encoding) {
  if(socket.writable){
...
Connection.prototype.write = function(data){
  ...
  if(this.readyState == 4){
    ...
    if(
      ...
      debug(this._id, "ERROR: write: "+data);
    }
  } else {
    debug(this._id, "ERROR: write: client not ready for write()ing")
  }
  return false;
};
