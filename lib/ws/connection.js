...
function Connection(server, req, socket, upgradeHead){
  ...
  if(server.options.datastore){
    ...
    if( "string" === typeof server.options.datastore ){
      storage = require(server.options.datastore);
    } else if(!!server.options.datastore && server.options.datastore !== true){
  ...
  if( !checkVersion(this)) {
    ...
    this.addListener("stateChange", function(state, laststate){
      ...
      if(state === 4){
        if(storage){
          connection.storage = new storage();
        }
