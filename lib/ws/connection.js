...
Connection.prototype.initializeParser = function(){
  ...
  this.socket.ondata = function(data, start, end){
    ...
    if(ws.readyState == 1) {
      ws.onmessage(slice);
    } else if(slice.substr(0,4) != "GET ") {
      ws.key3 = slice.replace("\r\n", "");
      ws.onConnect(ws.req);
    } else {
      Utils.debug(data.toString("binary", start, end));
      var parts = slice.split("\r\n\r\n");
      if(parts > 1){
        var body = parts[1].replace("\r\n", "");
        ws.key3 = body;
        ws.parser.execute(data, start, end-start-body.length);
      } else {
        ws.parser.execute(data, start, end-start);
      }
    }
  };
};
