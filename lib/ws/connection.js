...
function Connection(manager, options, req, socket, upgradeHead){
  ...
  if( !checkVersion(this)) {
    ...
    //      the upgradeHead.
    //
    // This switch is sorted by probability. 
    switch(this.version) {
      case "draft76":
...
var handshakes = {
  ...
  draft75: function(connection){
    ...
    if(location){
      ...
      var cli_prot = connection._req.headers['websocket-protocol'];
      var opt_prot = connection._options.subprotocol;

      if(opt_prot && typeof opt_prot == "string") {
	if (!cli_prot || (cli_prot == opt_prot)){
           res += "\r\nWebSocket-Protocol: "+ opt_prot;
	}else{
           connection.reject("Invalid WebSocket [Sub]Protocol.  This port handles '" + opt_prot + "' protocol messages .");
	}
      }else if (opt_prot && typeof opt_prot == "function") {
        res += "\r\nWebSocket-Protocol: " + opt_prot(connection, upgradeHead);
      }else if (cli_prot){
        res += "\r\nWebSocket-Protocol: " + cli_prot;
      }
  ...
  draft76: function(connection, upgradeHead){
    ...
    if(location){
     ...
     var cli_prot = connection._req.headers['sec-websocket-protocol'];
     var opt_prot = connection._options.subprotocol;

     if(opt_prot && typeof opt_prot == "string") {
	if (!cli_prot || (cli_prot == opt_prot)){
           res += "\r\nSec-WebSocket-Protocol: "+ opt_prot;
	}else{
           connection.reject("Invalid WebSocket [Sub]Protocol.  This port handles '" + opt_prot + "' protocol messages .");
	}
     }else if (opt_prot && typeof opt_prot == "function") {
        res += "\r\nSec-WebSocket-Protocol: " + opt_prot(connection, upgradeHead);
     }else if (cli_prot){
        res += "\r\nSec-WebSocket-Protocol: " + cli_prot;
     }
