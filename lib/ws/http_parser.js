...
var Parser = function (){
  ...
  parser.onMessageBegin = function () {
    parser.incoming = IncomingMessage();
    parser.field = null;
    parser.value = null;
  };
  ...
  parser.onURL = function (b, start, len) {
    var slice = b.toString('utf8', start, start+len);
  ...
  parser.onHeaderField = function (b, start, len) {
    var slice = b.toString('utf8', start, start+len).toLowerCase();
  ...
  parser.onBody = function (b, start, len) {
    parser.incoming.body = b.slice(start, start+len);    
  };

  parser.onMessageComplete = function () {
    if(parser["onEnd"]){
      parser.onEnd(parser.incoming);
    }
  };
...
module.exports = new FreeList('parsers', 1000, Parser);
